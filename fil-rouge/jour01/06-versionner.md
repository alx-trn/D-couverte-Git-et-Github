...
type : le type de code que je suis en train de réalisé
    ...
    - refactor => réoragnisation du code
...
git commit -a -m "..." ajouter une version incrémentale dans le reposiroty SI le fichier est déjà connu dans le repository

git statues // connaitre l'état de chaque fichier par rapport au repo local
git log local // history des commits
git log --oneline // version compact de l'historique
git checkout <...id_ancien_commit.1..> // revenir dans le passé
git checkout main // revenir dans le présent
